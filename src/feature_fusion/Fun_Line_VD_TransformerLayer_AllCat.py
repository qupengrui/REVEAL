...
CONFIG = {
    ...
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_AllFeaturesCat.txt",  # 训练日志路径
    "save_file": "/data/prqu/prqu_files/save_file",  # 结果保存目录
    "model_dir": "/data/prqu/prqu_files/save_file/models_metrics_modified_AllFeatureCat",
...
class VulDetectionModel(nn.Module):
    def __init__(self):
        ...
        # 行级分类头（直接拼接三种特征：行级语义特征 + 结构特征 + 全局特征）
        self.line_classifier = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model * 3, 512),  # 三种特征直接拼接，输入维度为 3*hidden
            nn.LayerNorm(512),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(512, 256),
    ...
    def forward(self, func_inputs, line_inputs, valid_mask):
        ...
        line_structural_features = self.line_transformer(
            all_line_features  # [batch, max_lines, hidden]
        )
        ...
        # 直接拼接三种特征：行级语义特征 + 结构特征 + 全局特征
        final_fused_features = torch.cat([all_line_features, line_structural_features, global_features_expanded],
                                         dim=-1)  # [batch, max_lines, 3*hidden]
...
def test(model, test_loader, device, writer):
    ...
    # 2、单独检测已保存模型的效果时
    best_model_path = "/data/prqu/prqu_files/save_file/models_metrics_modified_AllFeatureCat/epoch_035__f1_0.4349.pth"  # 使用F1最高的模型
    model.load_state_dict(torch.load(best_model_path, map_location='cpu', weights_only=True))
    model.to(CONFIG['device'])
