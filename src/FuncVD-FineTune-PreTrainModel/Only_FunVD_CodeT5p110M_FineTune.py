...
"""路径参数及超参设置（仅函数级）"""
CONFIG = {
    "func_bert_path": "/data/prqu/prqu_files/pretrained_models/CodeT5p_110M",
    "max_func_length": 512,
    "num_classes": 2,
    "batch_size": 16,
    "lr": 1e-5,
    ...
    "log_dir": "/data/prqu/prqu_files/dir_logs_test",
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_CodeT5P_func_only.txt",
    "save_file": "/data/prqu/prqu_files/save_file",
    "model_dir": "/data/prqu/prqu_files/save_file/models_func_only_CodeT5p_110M",
    "max_saved_models": 1,
}
...
class CodeDataset(Dataset):
    def __init__(self, df, max_func_length=512):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(CONFIG["func_bert_path"], trust_remote_code=True)
...
class VulDetectionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.codeT5P = AutoModel.from_pretrained(CONFIG["func_bert_path"], trust_remote_code=True)

        for param in self.codeT5P.parameters():
            param.requires_grad = False

        self._unfreeze_for_finetune()
    ...
    def _unfreeze_for_finetune(self):
        # 优先按 encoder.block 解冻最后6层；若结构不同则回退为全部解冻。
        if hasattr(self.codeT5P, "encoder") and hasattr(self.codeT5P.encoder, "block"):
            for param in self.codeT5P.encoder.block[-6:].parameters():
    ...
    def _infer_hidden_dim(self):
        for key in ("embed_dim", "d_model", "hidden_size"):
            if hasattr(self.codeT5P.config, key):
                return int(getattr(self.codeT5P.config, key))
        raise ValueError("无法从模型配置推断隐藏层维度")

    @staticmethod
    def _extract_embedding(outputs, attention_mask):
        ...
        counts = torch.sum(mask, dim=1).clamp(min=1.0)
        return summed / counts

    def forward(self, func_inputs):
        outputs = self.codeT5P(
            input_ids=func_inputs["input_ids"],
            attention_mask=func_inputs["attention_mask"],
        )
        global_features = self._extract_embedding(outputs, func_inputs["attention_mask"])
...
def test(model, test_loader, device, writer):
    if len(top_models) > 0:
        ...
        print("警告: 没有找到保存的 最佳模型，使用当前模型进行测试")

    test_loss, test_func_metrics = evaluate(model, test_loader, device, 0, writer, mode="test")
    write_logfile("Balanced_Test_Dataset_Metrics", 0, func_metrics=test_func_metrics, loss=test_loss)
