...
CONFIG = {
    ...
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_Trans+Pos_Structural+Global.txt",  # 训练日志路径
    "save_file": "/data/prqu/prqu_files/save_file",  # 结果保存目录
    "model_dir": "/data/prqu/prqu_files/save_file/models_trans+Pos_Structural+Global",
...
class VulDetectionModel(nn.Module):
    ...
    def forward(self, func_inputs, line_inputs, valid_mask):
        ...
        # Stage 4: 仅使用“行级结构特征 + 全局语义特征”进行融合
        # 将全局特征扩展到每一行
        global_features_expanded = global_features.unsqueeze(1).expand(-1, max_lines, -1)  # [batch, max_lines, hidden]
        
        # 最终特征融合：行级结构特征 + 全局特征
        final_fused_features = torch.cat([line_structural_features, global_features_expanded], dim=-1)  # [batch, max_lines, 2*hidden]
