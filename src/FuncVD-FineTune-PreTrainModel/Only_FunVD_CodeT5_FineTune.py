...
from transformers import RobertaTokenizer, T5ForConditionalGeneration, get_cosine_schedule_with_warmup
...
CONFIG = {
    ...
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_CodeT5_func_only.txt",
    "save_file": "/data/prqu/prqu_files/save_file",
    "model_dir": "/data/prqu/prqu_files/save_file/models_func_only_CodeT5",
    "max_saved_models": 1,
}
...
class CodeDataset(Dataset):
    def __init__(self, df, max_func_length=512):
        ...
        self.tokenizer = RobertaTokenizer.from_pretrained(CONFIG["func_bert_path"])
...
class VulDetectionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.codeT5 = T5ForConditionalGeneration.from_pretrained(CONFIG["func_bert_path"])

        for param in self.codeT5.parameters():
            param.requires_grad = False

        for param in self.codeT5.encoder.block[-6:].parameters():
            param.requires_grad = True

        self.func_classifier = nn.Sequential(
            nn.Linear(self.codeT5.config.d_model, 256),
            nn.LayerNorm(256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 128),
