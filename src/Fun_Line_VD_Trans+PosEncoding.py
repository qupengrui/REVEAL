...
CONFIG = {
    ...
    # 损失函数相关参数
    "alpha": 0.6,              # 函数级损失权重初始值
    "learnable_alpha": True,   # 是否使用可学习的alpha参数

    "log_dir": "/data/prqu/prqu_files/dir_logs_test",  # TensorBoard日志路径
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_Trans+PosEncoding_NewDataset_learnable_alpha.txt",  # 训练日志路径
    "save_file": "/data/prqu/prqu_files/save_file",  # 结果保存目录
    "model_dir": "/data/prqu/prqu_files/save_file/models_metrics_modified_Trans+PosEncoding_NewDateset",
...
class VulDetectionModel(nn.Module):
    def __init__(self):
        ...
        # 行间结构建模Transformer层
        # 注意：不使用src_key_padding_mask参数以避免PyTorch嵌套张量警告
        self.line_transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=self.codeT52.config.d_model,
                nhead=8,  # 8个注意力头
                dim_feedforward=self.codeT52.config.d_model * 2,  # FFN维度
                dropout=0.1,
                activation='relu',
                batch_first=True,  # 输入格式为 [batch, seq_len, features]
                norm_first=False  # 使用标准的post-norm结构
            ),
            num_layers=3,       # 3层Transformer编码器
            enable_nested_tensor=False  # 禁用嵌套张量以避免警告
        )

        # 行级语义特征融合层（行级特征 + 结构特征）
        self.line_semantic_fusion = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model * 2, self.codeT52.config.d_model),  # 原始带位置编码的行特征+结构特征，并进行降维
            nn.LayerNorm(self.codeT52.config.d_model),
            nn.ReLU(),
            nn.Dropout(0.1)
        )
        ...
        # 行级分类头（层级融合：语义特征 + 全局特征）
        self.line_classifier = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model * 2, 256),  # 融合语义特征+全局特征
            nn.LayerNorm(256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 128),
    ...
    def forward(self, func_inputs, line_inputs, valid_mask):
        ...
        # Stage 2: 使用 CodeT5-1 逐行提取行级特征 + 添加位置编码
        line_features_list = []
        ...
        # 堆叠所有行特征 [batch, max_lines, hidden] —— 该三维张量正式Transformer的标准输入形式
        all_line_features = torch.stack(line_features_list, dim=1)
        
        # 创建位置索引 [0, 1, 2, ..., max_lines-1]
        position_ids = torch.arange(max_lines, dtype=torch.long, device=device).unsqueeze(0).expand(batch_size, -1)  # [batch, max_lines]
        
        # 获取绝对位置编码
        position_embeddings = self.position_embedding(position_ids)  # [batch, max_lines, hidden]
        position_embeddings = self.position_dropout(position_embeddings)
        
        # 将位置编码与行语义特征融合——直接相加
        all_line_features_with_pos = all_line_features + position_embeddings  # [batch, max_lines, hidden]
        
        # 应用掩码：将填充行的位置编码置零
        valid_mask_expanded = valid_mask.unsqueeze(-1).expand_as(all_line_features_with_pos)  # [batch, max_lines, hidden]
        all_line_features_with_pos = all_line_features_with_pos * valid_mask_expanded.float()
        
        # Stage 3: 行间结构建模 - 使用Transformer建模行与行之间的结构关系
        # 通过Transformer编码器建模行间结构关系（不使用src_key_padding_mask避免警告）
        line_structural_features = self.line_transformer(
            all_line_features_with_pos  # [batch, max_lines, hidden] 使用加入位置编码的特征
        )
        
        # 手动应用掩码：将填充行的结构特征置零
        # valid_mask: [batch, max_lines], True表示有效行
        valid_mask_expanded = valid_mask.unsqueeze(-1).expand_as(line_structural_features)  # [batch, max_lines, hidden]
        line_structural_features = line_structural_features * valid_mask_expanded.float()
        
        # Stage 4: 行级语义特征融合（原始带行号编码的行特征 + 结构特征）
        # 拼接带位置编码的行特征和结构特征
        line_semantic_combined = torch.cat([all_line_features_with_pos, line_structural_features], dim=-1)  # [batch, max_lines, 2*hidden]
        
        # 通过融合层降维得到增强的行级特征
        enhanced_line_features = self.line_semantic_fusion(line_semantic_combined)  # [batch, max_lines, hidden]

        # Stage 5: 行级增强后的特征 与 全局特征融合 ———— 将全局特征扩展到每一行
        global_features_expanded = global_features.unsqueeze(1).expand(-1, max_lines, -1)  # [batch, max_lines, hidden]
        
        # 最终特征融合：增强的行级语义特征 + 全局特征
        final_fused_features = torch.cat([enhanced_line_features, global_features_expanded], dim=-1)  # [batch, max_lines, 2*hidden]
        
        # 行级分类
        line_logits = self.line_classifier(final_fused_features)  # [batch, max_lines, num_classes]
...
"""加权损失函数计算（添加可学习的alpha参数）"""
class MultiTaskLoss(nn.Module):
    def __init__(self, alpha=0.6, learnable_alpha=True):
        super().__init__()
    ...
    def get_normalized_alpha(self):
        """获取归一化后的alpha值（确保在[0,1]区间内）"""
        if self.learnable_alpha:
            return torch.sigmoid(self.alpha)  # 使用sigmoid确保在(0,1)区间
        else:
            return self.alpha
    ...
    def forward(self, outputs, labels):
        ...
        # 获取归一化的alpha值
        alpha_norm = self.get_normalized_alpha()

        # 加权总损失
        total_loss = alpha_norm * func_loss + (1 - alpha_norm) * line_loss

        return total_loss, func_loss, line_loss, alpha_norm


""""训练"""
def train_epoch(model, dataloader, optimizer, criterion, scheduler, device, epoch, writer):
    # 确保 model 和 criterion 均在训练模式
    model.train()
    criterion.train()
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # 计算损失
        batch_total_loss, batch_func_loss, batch_line_loss, alpha_current = criterion(logits, labels)

        # 累积损失
        total_loss += batch_total_loss.item()
        total_func_loss += batch_func_loss.item()
        total_line_loss += batch_line_loss.item()
        alpha_values.append(alpha_current.item())
    ...
    avg_func_loss = total_func_loss / len(dataloader)
    avg_line_loss = total_line_loss / len(dataloader)
    ...
    # 记录损失方面的分解变量，包括总体损失、函数级损失、行级损失 以及 损失权重阿尔法
    writer.add_scalars('Loss_Breakdown/Train',
                       {'Total': avg_total_loss,
                        'Function': avg_func_loss,
                        'Line': avg_line_loss,
                        'Alpha': avg_alpha}, epoch)

    return avg_total_loss, func_metrics, line_metrics


"""验证"""
def evaluate(model, dataloader, criterion, device, epoch, writer, mode='val'):
    # 验证时将model和criterion也设为eval模式
    model.eval()
    criterion.eval()
...
"""测试模块"""
def test(model, test_loader, criterion, device, writer):
    # 1、训练、验证、测试一体化时
    # 加载最佳模型
    if len(top_models) > 0:
    ...
    # # 2、单独测试已保存模型的效果时
    # best_model_path = "/data/prqu/prqu_files/save_file/models_metrics_modified_Trans+PosEncoding_NewDateset/epoch_042__f1_0.4623.pth"  # 使用F1最高的模型
    # model.load_state_dict(torch.load(best_model_path, map_location='cpu', weights_only=True))
    # model.to(CONFIG['device'])

    test_loss, test_func_metrics, test_line_metrics = evaluate(model, test_loader, criterion, device, 0, writer, mode='test')

    # 写入测试结果到日志文件
    write_logfile("Balanced_Test_Dataset_Metrics", 0, func_metrics=test_func_metrics, line_metrics=test_line_metrics, loss=test_loss)
    print_metrics('Test:', test_loss, test_func_metrics, test_line_metrics)


def test_CWE(model, criterion, device, writer,
             cwe_test_dir="/data/prqu/prqu_files/data/Most_Information_Dataset/test_with_cwe",
             result_txt_path=r"/data/prqu/prqu_files/save_file/test_CWE_results.txt",
             model_path="/data/prqu/prqu_files/save_file/models_metrics_modified_Trans+PosEncoding_NewDateset/epoch_042__f1_0.4623.pth"):
    """
    使用已保存模型对CWE子测试集逐一测试，并将结果统一保存到txt。
    ...
    Args:
        ...
        cwe_test_dir: CWE子测试集目录
        result_txt_path: 结果汇总txt路径
        model_path: 已保存模型路径
    """
    # 路径兼容：优先使用传入目录，不存在则回退到本地Windows目录
    candidate_dirs = [
        cwe_test_dir,
        r"/data/prqu/prqu_files/data/Most_Information_Dataset/test_with_cwe"
    ]
    ...
    if selected_dir is None:
        print("错误: 未找到CWE子测试集目录")
        print(f"已尝试目录: {candidate_dirs}")
        return None

    # 仅匹配按约定命名的CWE子集文件
    cwe_pattern = re.compile(r'^test_with_cwe_balanced_CWE(\d+)\.csv$', re.IGNORECASE)
    matched_files = []
    for file_path in Path(selected_dir).glob("*.csv"):
        match = cwe_pattern.match(file_path.name)
        if match:
            matched_files.append((int(match.group(1)), file_path))

    if not matched_files:
        print(f"错误: 目录中未找到CWE子集文件: {selected_dir}")
        return None
    ...
    # 加载保存模型
    print(f"加载测试模型: {model_path}")
    model.load_state_dict(torch.load(model_path, map_location='cpu', weights_only=True))
    model.to(device)

    # 准备结果文件目录
    result_dir = os.path.dirname(result_txt_path)
    if result_dir:
    ...
    with open(result_txt_path, "w", encoding="utf-8") as f:
        f.write("=" * 100 + "\n")
        f.write("Top10 CWE 子测试集评估结果\n")
        f.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"测试目录: {selected_dir}\n")
        f.write(f"模型路径: {model_path}\n")
        f.write("=" * 100 + "\n")

        for cwe_id, csv_path in matched_files:
            cwe_name = f"CWE-{cwe_id}"
            print(f"\n开始测试 {cwe_name}: {csv_path}")

            subset_df = pd.read_csv(str(csv_path), low_memory=False)
            sample_count = len(subset_df)

            if sample_count == 0:
                msg = f"[{cwe_name}] 样本数为0，跳过。\n"
                print(msg.strip())
                f.write(msg)
                continue

            subset_dataset = CodeDataset(
                subset_df,
                max_lines=CONFIG['max_lines'],
                max_func_length=CONFIG['max_func_length'],
            ...
            mode_name = f"test_{cwe_name.replace('-', '_')}"
            subset_loss, subset_func_metrics, subset_line_metrics = evaluate(
                model,
                subset_loader,
                criterion,
                device,
            ...
            all_results.append({
                ...
                'loss': subset_loss,
                'func': subset_func_metrics,
                'line': subset_line_metrics
            })

            result_block = (
                f"\n[{cwe_name}] 样本数: {sample_count}\n"
                f"Loss: {subset_loss:.4f}\n"
                f"[Func] Acc: {subset_func_metrics['acc']:.4f} | F1: {subset_func_metrics['f1']:.4f} | "
                f"Pre: {subset_func_metrics['precision']:.4f} | Rec: {subset_func_metrics['recall']:.4f}\n"
                f"[Line] Acc: {subset_line_metrics['acc']:.4f} | F1: {subset_line_metrics['f1']:.4f} | "
                f"Pre: {subset_line_metrics['precision']:.4f} | Rec: {subset_line_metrics['recall']:.4f} | "
                f"IoU: {subset_line_metrics['IoU']:.4f} | ROC-AUC: {subset_line_metrics['ROC-AUC']:.4f} | "
                f"PR-AUC: {subset_line_metrics['PR-AUC']:.4f}\n"
                f"[Top-K] Top-5%Acc: {subset_line_metrics['Top-5%Acc']:.4f} | "
                f"Top-10%Acc: {subset_line_metrics['Top-10%Acc']:.4f}\n"
            )
            f.write(result_block)
            print(f"{cwe_name} 测试完成。")

        if all_results:
            macro_func_f1 = np.mean([r['func']['f1'] for r in all_results])
            macro_line_f1 = np.mean([r['line']['f1'] for r in all_results])
            macro_line_iou = np.mean([r['line']['IoU'] for r in all_results])
            macro_line_top10 = np.mean([r['line']['Top-10%Acc'] for r in all_results])

            total_samples = sum(r['samples'] for r in all_results)
            weighted_line_f1 = sum(r['line']['f1'] * r['samples'] for r in all_results) / total_samples
            weighted_line_iou = sum(r['line']['IoU'] * r['samples'] for r in all_results) / total_samples

            summary_block = (
                "\n" + "=" * 100 + "\n"
                "汇总统计\n"
                f"CWE子集数量: {len(all_results)}\n"
                f"总样本数: {total_samples}\n"
                f"Macro-Func-F1: {macro_func_f1:.4f}\n"
                f"Macro-Line-F1: {macro_line_f1:.4f}\n"
                f"Macro-Line-IoU: {macro_line_iou:.4f}\n"
                f"Macro-Top-10%Acc: {macro_line_top10:.4f}\n"
                f"Weighted-Line-F1: {weighted_line_f1:.4f}\n"
                f"Weighted-Line-IoU: {weighted_line_iou:.4f}\n"
                + "=" * 100 + "\n"
            )
            f.write(summary_block)

    print(f"\nCWE子测试集测试完成，结果已保存到: {result_txt_path}")
    return all_results
...
if __name__ == "__main__":
    ...
    criterion = MultiTaskLoss(alpha=CONFIG["alpha"], learnable_alpha=CONFIG["learnable_alpha"])
    criterion = criterion.to(CONFIG['device'])

    # 初始化优化器，同时包含模型和损失函数的参数
    all_parameters = list(model.parameters()) + list(criterion.parameters())
    optimizer = torch.optim.AdamW(all_parameters, lr=CONFIG['lr'], weight_decay=0.1)
    ...
    for epoch in range(CONFIG['epochs']):
        """训练阶段"""
        print("\n==========开始训练阶段============\n")
        train_loss, train_func_metrics, train_line_metrics = train_epoch(model, train_loader, optimizer, criterion,
                                                                         scheduler, CONFIG['device'], epoch, writer)
        write_logfile("Train_Metrics", epoch, func_metrics=train_func_metrics, line_metrics=train_line_metrics,
                      loss=train_loss)

        """验证阶段"""
        print("\n==========开始验证阶段============\n")
        val_loss, val_func_metrics, val_line_metrics = evaluate(model, val_loader, criterion, CONFIG['device'], epoch,
                                                                writer)
    ...
    test(model, test_loader, criterion, CONFIG['device'], writer)

# =========================================新增实验，与上述训练、测试过程无关======================================================
    # # Top10 CWE子测试集评估，结果写入指定txt文件
    # test_CWE(
    #     model,
    #     criterion,
    #     CONFIG['device'],
