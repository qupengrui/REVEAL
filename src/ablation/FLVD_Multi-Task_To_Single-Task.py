...
CONFIG = {
    ...
    # 损失函数相关参数（单任务学习 - 已移除alpha参数）
    # "alpha": 0.6,              # 函数级损失权重初始值（多任务学习时使用）
    # "learnable_alpha": True,   # 是否使用可学习的alpha参数（多任务学习时使用）

    "log_dir": "/data/prqu/prqu_files/dir_logs_test",  # TensorBoard日志路径
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_Trans+Pos_Multi-Task_To_Single-Task_20.txt",  # 训练日志路径
    "save_file": "/data/prqu/prqu_files/save_file",  # 结果保存目录
    "model_dir": "/data/prqu/prqu_files/save_file/models_trans+Pos_Multi-Task_To_Single-Task_20",
...
class VulDetectionModel(nn.Module):
    ...
    def forward(self, func_inputs, line_inputs, valid_mask):
        ...
        # 单任务学习：只返回行级结果
        return {
            # 'func_logits': func_logits,  # 多任务学习时使用
...
"""写入训练日志至文件（单任务学习版本 - 只记录行级指标）"""
def write_logfile(phase, epoch, line_metrics, loss):
    log_entry = (
...
"""打印指标信息（单任务学习版本 - 只打印行级指标）"""
def print_metrics(phase, loss, line_metrics):
...
class SingleTaskLoss(nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        line_labels_flat = labels['line'].view(-1)          # [batch*lines]
        valid_mask = (line_labels_flat != -100)

        line_loss = F.cross_entropy(
            line_logits[valid_mask],
            line_labels_flat[valid_mask],
...
# """加权损失函数计算（添加可学习的alpha参数）- 多任务学习版本"""
...
def train_epoch(model, dataloader, optimizer, criterion, scheduler, device, epoch, writer):
    ...
    total_loss = 0.0
    # total_func_loss = 0.0  # 单任务学习不需要
    # total_line_loss = 0.0  # 单任务学习不需要
    # alpha_values = []  # 单任务学习不需要

    # 初始化预测和标签收集列表（单任务学习只收集行级）
    # func_preds, func_labels = [], []  # 单任务学习不需要
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # 记录每batch损失
        writer.add_scalar('Train/Batch_Loss', batch_loss.item(), epoch * len(dataloader) + batch_idx)

        # 打印行级标签分布，确保数据集中存在正样本（单任务学习）
...
def test(model, test_loader, criterion, device, writer):
    ...
    # # 2、单独测试已保存模型的效果时
    # best_model_path = "/data/prqu/prqu_files/save_file/" \
    #                   "/epoch_042__f1_0.4623.pth"  # 使用F1最高的模型
    # model.load_state_dict(torch.load(best_model_path, map_location='cpu', weights_only=True))
    # model.to(CONFIG['device'])

    test_loss, test_line_metrics = evaluate(model, test_loader, criterion, device, 0, writer, mode='test')

    # 写入测试结果到日志文件（单任务学习版本）
    write_logfile("Balanced_Test_Dataset_Metrics", 0, line_metrics=test_line_metrics, loss=test_loss)
    print_metrics('Test:', test_loss, test_line_metrics)
...
if __name__ == "__main__":
    ...
    criterion = SingleTaskLoss()  # 单任务学习：使用SingleTaskLoss
    criterion = criterion.to(CONFIG['device'])

    # 初始化优化器（单任务学习：损失函数没有可学习参数）
    ...
    for epoch in range(CONFIG['epochs']):
        """训练阶段"""
        print("\n==========开始训练阶段============\n")
        train_loss, train_line_metrics = train_epoch(model, train_loader, optimizer, criterion,
                                                     scheduler, CONFIG['device'], epoch, writer)
        write_logfile("Train_Metrics", epoch, line_metrics=train_line_metrics, loss=train_loss)
    
        """验证阶段"""
        print("\n==========开始验证阶段============\n")
        val_loss, val_line_metrics = evaluate(model, val_loader, criterion, CONFIG['device'], epoch, writer)
        write_logfile("Val_Metrics", epoch, line_metrics=val_line_metrics, loss=val_loss)
        ...
        print_metrics('Train:', train_loss, train_line_metrics)
        print_metrics('Val:', val_loss, val_line_metrics)
