...
CONFIG = {
    ...
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_Trans+Pos_AC+Cross_AB+Cross_EF+Cross.txt",  # 训练日志路径
    "save_file": "/data/prqu/prqu_files/save_file",  # 结果保存目录
    "model_dir": "/data/prqu/prqu_files/save_file/models_trans+Pos_AC+Cross_AB+Cross_EF+Cross",
...
class VulDetectionModel(nn.Module):
    def __init__(self):
        ...
        self.line_semantic_fusion = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model * 2, self.codeT52.config.d_model),  # 原始行特征+结构特征，并进行降维
        ...
        self.line_classifier = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model, 256),  # 使用最终G特征进行分类
            nn.LayerNorm(256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 128),
        ...
        # 交叉注意力模块：AB、AC、EF（三次跨模态融合）
        d_model = self.codeT52.config.d_model
        self.cross_ab = nn.MultiheadAttention(embed_dim=d_model, num_heads=8, dropout=0.1, batch_first=True)
        self.cross_ac = nn.MultiheadAttention(embed_dim=d_model, num_heads=8, dropout=0.1, batch_first=True)
        self.cross_ef = nn.MultiheadAttention(embed_dim=d_model, num_heads=8, dropout=0.1, batch_first=True)
    ...
    def forward(self, func_inputs, line_inputs, valid_mask):
        ...
        # Stage 4: 三段交叉注意力融合 (A,B,C -> E,F -> G)
        # A 作为单 token（K,V），与 B、C 分别交叉得到 E、F
        a_token = global_features.unsqueeze(1)  # [batch, 1, hidden]

        # AB: B 为 Q，A 为 K/V -> E
        e_attn, _ = self.cross_ab(query=all_line_features_with_pos, key=a_token, value=a_token)
        E = self.ln_e(all_line_features_with_pos + e_attn)  # 残差 + LN，形状 [batch, max_lines, hidden]

        # AC: C 为 Q，A 为 K/V -> F
        f_attn, _ = self.cross_ac(query=line_structural_features, key=a_token, value=a_token)
        F = self.ln_f(line_structural_features + f_attn)  # 残差 + LN，形状 [batch, max_lines, hidden]

        # EF: E 为 Q，F 为 K/V -> G
        # 使用无效行的 key_padding_mask 屏蔽 K/V 的无效位置
        key_padding_mask_f = ~valid_mask  # True 表示需要被mask掉
        g_attn, _ = self.cross_ef(query=E, key=F, value=F, key_padding_mask=key_padding_mask_f)
        G = self.ln_g(E + g_attn)  # 残差 + LN

        # 将无效行置零，保持与下游一致
        G = G * valid_mask.unsqueeze(-1).float()

        # 行级分类（使用最终 G 特征）
        line_logits = self.line_classifier(G)  # [batch, max_lines, num_classes]
