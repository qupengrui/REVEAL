...
CONFIG = {
    ...
    "log_file": "/data/prqu/prqu_files/save_file/training_logs_Trans+Pos_A+B_Then_C_CrossAttention.txt",  # 训练日志路径
    "save_file": "/data/prqu/prqu_files/save_file",  # 结果保存目录
    "model_dir": "/data/prqu/prqu_files/save_file/models_trans+pos_A+B_Then_C_CrossAttention",
...
class VulDetectionModel(nn.Module):
    def __init__(self):
        ...
        # 交叉注意力层——用于融合行级特征与全局特征
        # 使用MultiheadAttention实现交叉注意力
        self.cross_attention = nn.MultiheadAttention(
            embed_dim=self.codeT52.config.d_model,
            num_heads=8,
            dropout=0.1,
            batch_first=True  # 输入格式为 [batch, seq_len, features]
        )
        ...
        # 交叉注意力后的层归一化
        self.cross_attn_norm = nn.LayerNorm(self.codeT52.config.d_model)
        
        # 前馈网络（用于交叉注意力后的特征增强）
        self.cross_attn_ffn = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model, self.codeT52.config.d_model * 2),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(self.codeT52.config.d_model * 2, self.codeT52.config.d_model),
            nn.Dropout(0.1)
        )
        self.ffn_norm = nn.LayerNorm(self.codeT52.config.d_model)
        ...
        self.line_classifier = nn.Sequential(
            nn.Linear(self.codeT52.config.d_model, 256),  # 输入是交叉注意力融合后的特征
            nn.LayerNorm(256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 128),
    ...
    def forward(self, func_inputs, line_inputs, valid_mask):
        ...
        # Stage 4: 将带位置编码的行级语义特征与结构特征按位相加融合
        line_fused_features = all_line_features_with_pos + line_structural_features  # [batch, max_lines, hidden]
        
        # 应用掩码：将填充行的融合特征置零
        valid_mask_expanded = valid_mask.unsqueeze(-1).expand_as(line_fused_features)
        line_fused_features = line_fused_features * valid_mask_expanded.float()

        # Stage 5: 使用交叉注意力融合行级特征与全局特征，将全局特征扩展为与行级特征相同的序列长度，用作Key和Value
        global_features_expanded = global_features.unsqueeze(1).expand(-1, max_lines, -1)  # [batch, max_lines, hidden]
        
        # 交叉注意力：Query是行级融合特征，Key和Value是全局特征
        # 这样可以让每一行的特征都能关注到全局信息
        cross_attn_output, _ = self.cross_attention(
            query=line_fused_features,           # [batch, max_lines, hidden]
            key=global_features_expanded,        # [batch, max_lines, hidden]
            value=global_features_expanded,      # [batch, max_lines, hidden]
            key_padding_mask=~valid_mask         # [batch, max_lines] True表示需要mask的位置
        )
        
        # 残差连接 + 层归一化
        cross_attn_output = self.cross_attn_norm(line_fused_features + cross_attn_output)
        
        # 前馈网络增强特征 + 残差连接
        ffn_output = self.cross_attn_ffn(cross_attn_output)
        final_fused_features = self.ffn_norm(cross_attn_output + ffn_output)  # [batch, max_lines, hidden]
        
        # 最终应用掩码
        valid_mask_expanded = valid_mask.unsqueeze(-1).expand_as(final_fused_features)
        final_fused_features = final_fused_features * valid_mask_expanded.float()
